...
EMPLOYEE = "逸然"  # 請根據實際情況設定
SHARED_FOLDER = "/Volumes/助理美工區/#國軒"

# 將此處改成你自己的 ngrok 公開網址
NGROK_URL = 'https://ecc9-61-216-19-45.ngrok-free.app'
...
def on_drop(event):
    ...
    # 計算有效日期：凌晨 0-6 點歸屬前一日
    if current_hour < 6:
        effective_date = (now - timedelta(days=1)).date()
    else:
        effective_date = now.date()
    today_str = effective_date.strftime("%Y-%m-%d")
    month_str = effective_date.strftime("%Y-%m")

    # 判斷班別：假設每日 14:00～23:00 為正班，其餘為加班
